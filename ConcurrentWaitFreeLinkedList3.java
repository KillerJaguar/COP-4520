...
public class ConcurrentWaitFreeLinkedList3<E>
	//implements WaitFreeLinkedList<E>
{
	...
	final AtomicReferenceArray<State>threadState;
	
	// Current highest phase number 
	AtomicLong phaseNumber;
	
	@SuppressWarnings("unchecked")
	public ConcurrentWaitFreeLinkedList3(int numThreads)
	{
		...
		this.threadState = new AtomicReferenceArray<State>(numThreads);
		this.phaseNumber = new AtomicLong(0);
	}
	
	public void insert(int threadIdHelping, E item)
	{
		operate(threadIdHelping, Operation.INSERT, item);
	}
	
	public void delete(int threadIdHelping, E item)
	{
		operate(threadIdHelping, Operation.DELETE, item);
	}
	
	// WAIT-FREE
	public boolean contains(int threadIdHelping, E item)
	{
	...
	void operate(int threadIdHelping, Operation operation, E item)
	{
		long phaseNumber = this.phaseNumber.getAndIncrement();

		
		State st = new State(phaseNumber, operation, item);
		threadState.set(threadIdHelping, st);
		
		for(int i = 0; i < threadState.length(); i ++){
			State state = threadState.get(i);
	...
	Window find(E item)
	{
		...
		retry: while(true)
		{
			...
			while (true)
			{
				...
				if (item.hashCode() >= succ.item.hashCode() || item.equals(succ.item))//this line can produce a null pointer exception
